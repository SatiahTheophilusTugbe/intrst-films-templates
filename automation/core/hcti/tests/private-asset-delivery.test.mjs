...
assert.throws(() => assertHctiRequestSize(measurements[0].request_bytes), (error) => error.code === "HCTI_REQUEST_LIMIT_UNPROVEN");
assert.equal(assertHctiRequestSize(measurements[0].request_bytes, measurements[0].request_bytes), true);
...
assert.throws(() => assertRuntimeDeliveryReady({ credentialBindingStatus: "bound" }), (error) => error.code === "HCTI_REQUEST_LIMIT_UNPROVEN");
...
console.log("private asset delivery tests: 20 passed");
