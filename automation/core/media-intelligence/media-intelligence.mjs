...
export function buildCacheKey({ platform = "youtube", resourceType, externalId, language = null, sample = null }) {
  if (!resourceType || !externalId) fail("CACHE_KEY_INVALID", "resourceType and externalId are required.");
  const normalized = JSON.stringify({ platform, resourceType, externalId, language, sample });
  return `media:v1:${createHash("sha256").update(normalized).digest("hex")}`;
}
