...
const forbidden = new Set(["n8n-nodes-base.webhook", "n8n-nodes-base.scheduleTrigger", "n8n-nodes-base.wait"]);
...
if (!JSON.stringify(workflow).includes("httpHeaderAuth")) throw new Error("HTTP credential contract missing");
...
if (!JSON.stringify(workflow).includes("LIVE_BLOCKED_UNTIL_ATOMIC_CLAIM_AND_PUBLICATION_APPROVAL") || !JSON.stringify(workflow).includes("automatic_retries") || !JSON.stringify(workflow).includes("Credential and account readiness")) throw new Error("credential-gated terminal contract missing");
