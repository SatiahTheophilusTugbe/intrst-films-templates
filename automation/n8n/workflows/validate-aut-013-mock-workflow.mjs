...
export function validateWorkflow(workflow) {
  ...
  if (!workflow.meta || workflow.meta.project_id !== PROJECT_ID || workflow.meta.environment !== "development" || workflow.meta.mock_transport_only !== true || workflow.meta.provider_calls !== 0 || workflow.meta.provider_credits_consumed !== 0) fail("development/mock/project declarations are invalid");
  if (!Array.isArray(workflow.tags) || !workflow.tags.some((tag) => tag?.name === "project:intrst")) fail("required project tag is missing");
  ...
  if (!workflow.nodes.find((node) => node.name === "Mock atomic claim")?.parameters?.jsCode.includes("status: 'CLAIMED'") || !workflow.nodes.find((node) => node.name === "Exactly one MockTranscriptTransport invocation")?.parameters?.jsCode.includes("request_count: 1")) fail("mock claim/transport are not structurally enforced");
  return true;
}
